...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Object object = new Object((int)world.x(), (int)world.y(), 60, 30);
        ...
        Player player = new Player((int)camera.target().x(), (int)camera.target().y(), 15, 20, false);
...
//            }
            ...
            if(!player.CollisionWithUp) {
                if (IsKeyDown(KEY_W)) {
                    camera.target().y(camera.target().y() + (-velosity) * GetFrameTime() * 1000);
                }
            }
            //}
            if(!player.onFoor) {
           ...
           // if(!player.onFoor){
            ...
            if(!player.CollisionWithLeft) {
                if (IsKeyDown(KEY_A)) {
                    camera.target().x(camera.target().x() + (-velosity) * GetFrameTime() * 1000);
                }
            }
